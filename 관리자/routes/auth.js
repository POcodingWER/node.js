...
//회원정보수정불러오기
router.get('/userinfo',isLoggedIn, async(req, res) => {try{
  const user = await User.findAll();
  res.render('userinfo', { title: '개인정보 수정 - NodeBird',users : user });
  }catch(err){
      console.error(err);
      next(err);
  }
...
router.post('/userinfo',isLoggedIn, async(req, res) => {
  const {userId, nick, userDate, postcode, address, detailAddress, extraAddress, userTel, userMail,} = await req.body;
  try{
